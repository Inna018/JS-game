...
class Lever {
  ...
  constructor() {
    this.fountainImg = objectsArray.filter(
      (elem) =>
        elem.outerHTML.split('"')[1] === "assets/3 Objects/Fountain/2.png"
    )[0];
    this.x = heroX - 20;
    this.y = heroY;
...
class Enemy {
  ...
  showHurt() {
    ...
    text.style.left =
      this.dir < 0
        ? Number.parseInt(this.block.style.left) + 50
        : Number.parseInt(this.block.style.left) + 10;
...
const addStartScreen = () => {
  ...
  btn.addEventListener("click", () => {
    ...
    let cutscene = new Cutscene([
      "Після невдалої спроби вистежити викрадачів своєї дівчини, Адам був спійманий недоброзичливцями. Вони вирішили протестувати на героя нещодавно вкрадену надсекретну розробку. У результаті - свідомість Адама було укладено у віртуальний полон. Весь цей час друзі героя шукали його і через кілька днів, нарешті змогли вийти з ним на зв'язок.",
      "Виявилося, що з віртуального світу можна втекти – двері перебувати за одним із фонтанів наприкінці першого рівня. Але щоб її відкрити потрібно знайти захований важіль і ввести код пароля. \n\nПароль складається з 4 чисел. Цифри пароля знаходяться всередині дерев'яних ящиків, що ретельно охороняються (по одній у кожному). Що стосується важеля - він захований на другому рівні, куди у Адама немає доступу.",
    ]);
  });
  div.appendChild(btn);
};
